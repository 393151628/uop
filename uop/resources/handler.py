...
from uop.models import ResourceModel, Statusvm,OS_ip_dic,Deployment,DBIns, ComputeIns
    ...
    try:
        ...
        if len(resources_obj):
            ...
            if reservation_status in ["set_fail","unreserved","approval_fail","revoke","reserving"]:
                if reservation_status in ["reserving","set_fail"] and cloud == "2" and resource_type == "app":
                    resources.reservation_status = "deleting"
                    resources.save()
                else:
                    deleted_date = datetime.datetime.now()
                    new_resource_name = resources.resource_name + '_' + str(deleted_date)[20:-1]
                    resources.update(resource_name=new_resource_name,is_deleted=1,deleted_date=deleted_date,reservation_status="delete_success")
            else:
                resources.reservation_status = "deleting"
                resources.save()
...
def resource_post(resource_info_list):
        ...
        for info in resource_info_list:
            ...
            if resource_type in ["app", "kvm", "mysql", "redis", "mongodb"]:
                res_count = ResourceModel.objects.filter(project_name=project_name, resource_type=resource_type,
                                                         business_name=business_name, module_name=module_name, env=env,
                                                         is_deleted=0).count()
                if res_count > 0:
    ...
    for info in resource_info_list:
        ...
        resource_application = ResourceModel(resource_name=resource_name, project=project, department=department,
                                             ...
                                             expiry_date=expiry_date, admin_emails=admin_emails,
                                             user_emails=user_emails, cc_emails=cc_emails, mail_content=mail_content,
        ...
        try:
            for insname in ins_name_list:
                if ResourceModel.objects(compute_list__match={'ins_name': insname}).filter(env=env,
                                                                                           is_deleted=0).count() > 0:
                    code = 200
                    res = {
                        'code': code,
                        'result': {
